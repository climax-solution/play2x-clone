@if(Auth::guest()) <script type="text/javascript">load('/', function() { $('.md-auth').toggleClass('md-show', true); });</script> @else
<div class="bonus_page_wrapper">
    <div class="bonus_page_header">Special offers and bonuses</div>
    <div class="bonus_page_container">
        <div class="bonus_option" onclick="$('#wheel').toggleClass('md-show', true)">
            <div class="bonus_option_content">
                <div class="bonus_option_image bonus_option_bonus_wheel"></div>
                <div class="bonus_option_name">Wheel</div>
                <div class="bonus_option_description">Spin the free bonus wheel<br>every 3 minutes!</div>
                <div class="bonus_option_button">Открыть</div>
            </div>
        </div>
        <div class="bonus_option" onclick="$('#ref').toggleClass('md-show', true)">
            <div class="bonus_option_content">
                <div class="bonus_option_image bonus_option_bonus_ref"><div></div></div>
                <div class="bonus_option_name">Affiliate program</div>
                <div class="bonus_option_description">Get a free bonus<br>for every 10 referrals!</div>
                <div class="bonus_ref_option_counter"><span class="ref_reload_text">0/10</span>&nbsp;Referrals</div>
                <div class="bonus_option_button">More details</div>
            </div>
        </div>

        @if(strpos(Auth::user()->login, 'id') !== false)
            <div class="bonus_option" onclick="window.open('https://vk.com/bets.play2x')">
                @if(\App\User::isSubscribed(Auth::user()->login2))
                    <div class="bonus_option_ok">
                        <i class="fal fa-check"></i>
                        <span>You have completed this task.</span>
                    </div>
                @endif
                <div class="bonus_option_content">
                    <div class="bonus_option_image bonus_option_vk"><i class="fab fa-vk"></i></div>
                    <div class="bonus_option_name">Vkontakte community</div>
                    <div class="bonus_option_description">Join the VKontakte group and get 0.45 &nbsp;<i class="fad fa-coins"></i>to the account!</div>
                    <div class="bonus_option_button">Go</div>
                </div>
            </div>
        @endif
        <div id="notificationOption" style="display: none" class="bonus_option">
            @if(Auth::user()->notify_bonus == 1)
                <div class="bonus_option_ok">
                    <i class="fal fa-check"></i>
                    <span>You have completed this task.</span>
                </div>
            @endif
            <div class="bonus_option_content">
                <div class="bonus_option_image bonus_option_notify"><i class="fas fa-bell"></i></div>
                <div class="bonus_option_name">Notifications</div>
                <div class="bonus_option_description">Subscribe to notifications with promotional codes and news and get 1 &nbsp;<i class="fad fa-coins"></i> on the bill!</div>
                <div class="bonus_option_button">Subscribe</div>
            </div>
        </div>
    </div>
    <div class="bonus_page_header">Events</div>
    <div class="bonus_page_container">
        <div class="bonus_option" onclick="window.open('https://vk.com/bets.play2x')">
            <div class="bonus_option_content">
                <div class="bonus_option_image bonus_option_vk_m"><i class="fal fa-handshake"></i></div>
                <div class="bonus_option_name" style="width: 75%;">Cooperation</div>
                <div class="bonus_option_description" style="margin-bottom: 64px;">Are you an administrator of a VKontakte group? We can offer daily unique promotional codes for your group.</div>
                <div class="bonus_option_button">Connect with us</div>
            </div>
        </div>
        <div class="bonus_option" onclick="$('#rain').toggleClass('md-show', true)">
            <div class="bonus_option_content">
                <div class="bonus_option_image bonus_option_rain">
                    <img src="/img/game/svg/clouds.svg" alt="">
                </div>
                <div class="bonus_option_name">Rain</div>
                <div class="bonus_option_description">
                    Every few hours<br>an event occurs in the chat that<br>gives out a bonus to<br>random people.
                </div>
                <div class="bonus_option_button">More details</div>
            </div>
        </div>
        <div class="bonus_option" onclick="$('.md-promo').toggleClass('md-show', true)">
            <div class="bonus_option_content">
                <div class="bonus_option_image bonus_option_promo">
                    <img src="data:image/png;base64," alt="">
                    <img src="data:image/png;base64," alt="">
                </div>
                <div class="bonus_option_name">Promo code</div>
                <div class="bonus_option_description">Enter a promotional code or referral code and receive a bonus to your account!</div>
                <div class="bonus_option_button">Enter</div>
            </div>
        </div>
    </div>
</div>
@endif
